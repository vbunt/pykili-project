...
def get_education(table): # this is where  we do things with 'education' column
    for j in range(7879): # for every person in the table
        ...
        age, first_edu_year, first_edu_degree, first_edu_school, last_edu_year, highest_degree, hse_spec = '', '', '', '', '', '', '' # interesting things
        ...
        if not isNaN(raw):
            ...
            for i in range(len(line)):
                ...
                if i % 2 == 0 and line[i].isnumeric(): # we check that el is a year, after year they always have the degree and institution
                    ...
                    if 'Высшая школа экономики' in alma_mater: # first specialty for those who attended HSE
                        spec = re.findall('специальность «([^»]+)»', good_stuff)
                        if spec:
                            spec = spec[0]
                    edu[line[i]] = [degree, alma_mater, spec]

            if edu: # get interesting stuff
                ...
                for year in edu.keys():
                    if 'Высшая школа экономики' in edu[year][1]:
                        hse_spec = edu[year][2]
                ...
                #print(age, first_edu_school, hse_fac)
